...
Using some simple bar plots, the huge increase in heroin offenses is immediately obvious. We have seen a staggering *`r round(per_change_heroin$count[2]/per_change_heroin$count[1]* 100, 0)`%* increase in heroin offenses from 2007 to 2016. While it is true we have seen an increase in methamphetamine and opiate offenses--by *`r round(per_change_meth$count[2]/per_change_meth$count[1]* 100, 0)`%* and *`r round(per_change_opiates$count[2]/per_change_opiates$count[1]* 100, 0)`%* respectively-- it is not to nearly the same degree. Heroin's offenses have increased over 3 times as fast as those of meth and other opiates, even though we are seeing large increases in crime related to those drugs too!
...
In that vein, we have to at least mention the changes in the opiate drug landscape that could explain some of this heroin epidemic. Back in 2010 and 2011, two of the major opiates abused recreationally were [reformulated](https://odcp.ky.gov/Pages/The-Heroin-Epidemic.aspx) to make recreational use more difficult. Around the same time, various systems(for instance KASPER) were put in place to make abusing prescription opiates more difficult. Though it is always difficult to establish causation, it seems very likely that effectively restricting the supply of prescription opiates around 2011 triggered a surge in heroin demand as a replacement.
...
Using a shapefile from [this](http://data.lojic.org/datasets/jefferson-county-ky-zip-codes) website, I grabbed the zip code boundaries for the city of Louisville and after some cleaning, aggregation, and a *ton* of formatting(huge thanks to [Timo Grossenbacher](https://timogrossenbacher.ch/2016/12/beautiful-thematic-maps-with-ggplot2-only/) and his brilliant post on formatting clean choropleth plots) we can easily see the spread of heroin use through the city.
...
heroin_map_df <- readRDS(here::here("Data", "heroin_map_df.rds.gzip"))
# Base map theme
theme_map <- function(...) {
  theme_minimal() +
    theme(
...
extendLegendWithExtremes <- function(p, data){
  ...
  legend_grobs <- legend$grobs[[3]]$grobs[[1]]
...
changes_by_zip <- heroin_zip%>%
  ...
  spread(key = year, value = value)%>%
  mutate(per_change = `2016`/`2008` * 100)
...
But perhaps even more shocking is that even the 'better' parts of the city are being hit. The east end and regions like Prospect-- typically thought of as very affluent parts of the city-- have seen huge increases as well. We see east end regions with increases as high as `r hurstbourne$per_change`%. This is definitely not something that is only happening in one area of the city. An incredible `r round(sum(greater_than$greater_than,na.rm = TRUE)/nrow(greater_than)*100, 1)`% of Louisville zip codes are seeing at least 1 heroin crime every other week. Keep in mind that this is heroin *crime* data--meaning that heroin *use* is likely to be significantly higher in all regions. I shudder to think about the true number of overdoses or the number of families affected by the epidemic.
