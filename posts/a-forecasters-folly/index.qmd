...
full_raw <- read_rds(here::here("Data", "full_raw.RDS"))
```
...
As you can see, the mean model performs best, with a MAPE(Mean Absolute Percentage Error) of `r round(acc_df$MAPE[1], 1)`%. This will be the baseline error rate for our models to beat. As you'll see, it's not always easy to beat this metric.
...
The trend term is just the linear models calculation of the general upward or downward motion of the data. You can get a good visualization of both the seasonal and trend components by [decomposing](http://otexts.org/fpp2/classical-decomposition.html) the time series.
...
where ${e}_{t}$ is a noise term and the rest is a multiple regression model but with lagged values (i.e. ${y}_{t-1}$) as predictors.

The moving average model takes a different approach and uses forecast errors to build a model. Formally, the moving average model is written as 

$${y}_{t} = {c} + {e}_{t} + {\theta}_{1}{e}_{t-1} + {\theta}_{2}{e}_{t-2} + ... + {\theta}_{q}{e}_{t-q}$$
...
This is all a vast simplification of the entire process, but a more in depth discussion is beyond the scope of this post. For a great in depth discussion of what this model involves you can look [here](http://otexts.org/fpp2/ch-arima.html).
